...
describe('PolymarketService', () => {
    let service;
    beforeEach(() => {
        service = new polymarketService_1.PolymarketService({
            baseURL: BASE_URL,
    ...
    it('returns raw market data when the API responds with a markets array', async () => {
        ...
        const scope = (0, nock_1.default)(BASE_URL).get('/markets').reply(200, { markets });
    ...
    it('throws a PolymarketApiError when the upstream API fails', async () => {
        const scope = (0, nock_1.default)(BASE_URL).get('/markets').reply(500, { message: 'Upstream failure' });
        await expect(service.getMarkets()).rejects.toBeInstanceOf(polymarketService_1.PolymarketApiError);
        expect(scope.isDone()).toBe(true);
    });
    it('attaches authentication headers when provided', async () => {
        const authService = new polymarketService_1.PolymarketService({
            baseURL: BASE_URL,
            tradesBaseURL: TRADES_BASE_URL,
            auth: {
    ...
    it('fetches trades using market hash', async () => {
        ...
        const scope = (0, nock_1.default)(TRADES_BASE_URL)
            .get('/trades')
    ...
    it('throws when neither marketId nor marketHash provided for trades', async () => {
        await expect(service.getTrades({ limit: 5 })).rejects.toThrow(/marketId or marketHash/i);
    });
});
//# sourceMappingURL=polymarketService.test.js.map